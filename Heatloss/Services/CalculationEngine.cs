...
namespace QOVETER.Services
{
    public class CalculationEngine
    {
        ...
        public List<CalculationResult> Calculate(List<RoomData> rooms, CalculationParameters parameters, BuildingParameters buildingParams)
        {
            ...
            // Бытовые тепловыделения — постоянный источник расхождения с расчётом
            // проектировщика: формула (1) ТЗ идёт с вычетом, а для подбора приборов
            // отопления вычет обычно снимают. Разница на реальном этаже — около 16%
            // итога, и в журнале должно быть видно, по какой из двух формул считано.
            double internalHeat = computations.Sum(c => c.QVn);
            Logger.Info(parameters.SubtractInternalHeatGains
                ? $"[Бытовые] ВЫЧТЕНЫ из итога по формуле (1) ТЗ: {internalHeat:F0} Вт " +
                  "по выбранным помещениям."
                : $"[Бытовые] НЕ вычитаются: {internalHeat:F0} Вт по выбранным помещениям " +
                  "остаются в итоге (режим подбора отопительных приборов). " +
                  "При сверке с расчётом проектировщика это первое, что нужно уравнять: " +
                  "включите «Вычитать бытовые (для сверки)».");
        ...
        /// <summary>
        /// Со скольких уровней номер квартиры считается ПОВТОРЯЮЩИМСЯ, а не
        /// двухуровневой квартирой. Два уровня бывают у настоящего дуплекса,
        /// три — практически никогда.
        /// </summary>
        private const int MinLevelsForRepeatedNumbering = 3;
        ...
        /// в расчёте.</para>
        ///
        /// <para><b>Критерий — два признака, любого достаточно.</b></para>
        ///
        /// <list type="number">
        /// <item>Хотя бы один номер встречается на ТРЁХ и более уровнях. Квартира
        /// в три уровня — исчезающая редкость, а номер на трёх этажах доказывает,
        /// что номер в этом проекте не уникален по дому. Одного такого номера
        /// достаточно: нумерация — свойство проекта, а не отдельной квартиры.</item>
        /// <item>Больше половины номеров встречается более чем на одном уровне.
        /// Двухуровневых квартир большинством не бывает.</item>
        /// </list>
        ///
        /// <para><b>Почему одного большинства мало.</b> В моделях под одним
        /// «номером квартиры» лежат и общедомовые помещения — под составными
        /// номерами вида «секция-этаж-номер», уникальными по дому. Таких номеров
        /// по одному на этаж, то есть много, и они разбавляют счёт: 13 повторяющихся
        /// номеров против полусотни уникальных дают меньшинство, и признак
        /// не срабатывал бы ровно там, где нужен.</para>
        ///
        ...
        internal static bool UsesPerLevelNumbering(IList<RoomData> rooms)
        {
            ...
            var byNumber = rooms
                ...
                .Select(g => new { Number = g.Key, Levels = g.Select(r => r.LevelId).Distinct().Count() })
                .ToList();

            if (byNumber.Count == 0) return false;

            int multiLevel = byNumber.Count(g => g.Levels > 1);
            int repeating  = byNumber.Count(g => g.Levels >= MinLevelsForRepeatedNumbering);

            bool perLevel = repeating > 0 || multiLevel * 2 > byNumber.Count;

            Logger.Info(
                $"[Квартиры] номеров {byNumber.Count}: на нескольких уровнях {multiLevel}, " +
                $"на трёх и более {repeating} → нумерация " +
                (perLevel
